...
def is_available_to_take_out_only_red_marble(game_map):
    # 최단거리, 가능한 것 모두 해야함 -> BFS -> Queue, visited 필요
    # 0. queue.popleft()
    # 만약 -1
    # 1. 4방향 가지치기
    #   (1) count +1
    #   (2) 가면서 구멍 있는지 확인
    #       i) 있 -> B도 빠지는지 확인 -> 같이 빠지면 return -1 아니면 return count
    #       ( ii) 없 -> 계속함 )
    #   (3) visited 1 아니면, 도착한 위치와 count를 같이 queue에 추가, visited = 1처리
    #       맞으면, 일단 4방향 무조건 집어 넣음 (벽이면 -1 or None).
    # count 10 넘으면 그만.

    # queue: 현위치, count수 같이 기록.
    # 걱정: a,b, 같은 방향으로 같이 가야하는데 싱크 엇나가지는 않을지. -> 벽이라도 무조건 집어넣기로 대비.
